...
#else
  int16_t l_enc_prev = wheel_l_.enc;
  int16_t r_enc_prev = wheel_r_.enc;

  comms_.read_encoder_values(wheel_l_.enc, wheel_r_.enc);
  double delta_seconds = period.seconds();
  int16_t l_delta = wheel_l_.enc - l_enc_prev;
  int16_t r_delta = wheel_r_.enc - r_enc_prev;
  double l_delta_angle = (((double)l_delta) * wheel_l_.rads_per_count);
  double r_delta_angle = (((double)r_delta) * wheel_r_.rads_per_count);
  wheel_l_.pos += l_delta_angle;
  wheel_r_.pos += r_delta_angle;
  wheel_l_.vel = l_delta_angle / delta_seconds;
  wheel_r_.vel = r_delta_angle / delta_seconds;
